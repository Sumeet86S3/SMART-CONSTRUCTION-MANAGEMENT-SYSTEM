...
def evaluate_risk(results, model):
    detected_classes = []
    if len(results[0].boxes) > 0:
        class_indices = results[0].boxes.cls.cpu().numpy()
        detected_classes = [model.names[int(c)] for c in class_indices]

    helmet_present = "Helmet" in detected_classes
    vest_present = "Vest" in detected_classes

    if not helmet_present and not vest_present:
        return "CRITICAL", "Both helmet and vest are missing."
    if not helmet_present:
        return "HIGH RISK", "Helmet is missing."
    if not vest_present:
        return "MEDIUM RISK", "Vest is missing."
    return "LOW RISK", "Helmet and vest are present."
...
if input_type == "Image":
    ...
    if uploaded_image:
        ...
        risk_level, risk_message = evaluate_risk(results, model)
        st.markdown(f"**Risk Level:** {risk_level}")
        st.markdown(f"**Notes:** {risk_message}")
    ...
    camera_image = st.sidebar.camera_input("Take a picture using your webcam")
    if camera_image:
        image = Image.open(camera_image)
        st.image(image, caption="Camera Image", use_column_width=True)

        st.subheader("Detection Results:")
        annotated_image, results, image_output_path = detect_image(image, model, original_filename="camera")
        st.image(annotated_image, caption="Detected Image", use_column_width=True)
